...
contract('SitisArbitration', function(accounts) {
  ...
  arbiterList = [accounts[0], accounts[1], accounts[2]]
  noInArbiter = accounts[3]
  ...
  it("Проверяем чтение контракта", async function() {
    await this.arbitration.createArbitration(2, 2, 1, 20, 1)
    await this.arbitration.createArbitration(3, 3, 1, 99, 12)
    const cnt_created = await this.arbitration.arbiterCount()
    cnt_created.should.be.bignumber.equal(2)
    return this.arbitration.arbiterStats.call(2).then(function (res) {
      expect(res[0].toNumber()).to.be.equal(2)
      expect(res[1].toNumber()).to.be.equal(3)
      expect(res[2].toNumber()).to.be.equal(3)
